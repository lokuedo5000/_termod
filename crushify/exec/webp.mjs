...
// Formatos de imagen soportados para convertir a WEBP
...
class ImageConverter {
  ...
  async processFile(filePath, options) {
    try {
      ...
      const outputPath = path.join(options.dest, `${fileName}.webp`);

      // Configurar opciones de conversión para WEBP
      const webpOptions = {
        quality: options.quality || 80, // WEBP funciona mejor con calidad 80 por defecto
        lossless: options.lossless || false, // Por defecto usar compresión con pérdida
        effort: options.effort || 4, // Nivel de esfuerzo de compresión (0-6)
        nearLossless: options.nearLossless || false, // Compresión casi sin pérdida
        alphaQuality: options.alphaQuality || 100, // Calidad del canal alfa
      };
...
export default async function toWEBP(event, args) {
